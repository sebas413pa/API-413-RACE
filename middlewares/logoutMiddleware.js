...
const authenticateLogout = async (req, res, next) => {
    ...
    logger.info('Logout realizado con exito');
    next();
    }catch(error){
        logger.error('Error al realizar el logout', error);
        res.status(500).json(response.errorResponse('Error al realizar el logout', error));
    }
